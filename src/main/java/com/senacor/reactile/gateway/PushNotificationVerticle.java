...
public class PushNotificationVerticle extends AbstractVerticle {
    ...
    public static final String PUBLISH_ADDRESS_BRANCH_ADDRESS_UPDATE = "PushNotification#Branch#updateAddress#branchId=";
    ...
    public void start() {
        ...
        registerBranchAddressUpdateHandler();
    }
    ...
    private void registerBranchAddressUpdateHandler() {
        vertx.eventBus().consumer(BranchService.ADDRESS_EVENT_UPDATE_ADDRESS).toObservable()
                .map(Message::body)
                .cast(JsonObject.class)
                .subscribe(updateEvent -> {
                    String publishAddress = PUBLISH_ADDRESS_BRANCH_ADDRESS_UPDATE + updateEvent.getString("id");
                    logger.info("publish event on Address: " + publishAddress);
                    vertx.eventBus().publish(publishAddress, updateEvent);
                }, throwable -> logger.error("Error while handling event from " + BranchService
                        .ADDRESS_EVENT_UPDATE_ADDRESS, throwable));
    }
